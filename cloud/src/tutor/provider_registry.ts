...
import { createWorkersAITurnProvider, type WorkersAIBinding } from './workers_ai_provider';
...
export function resolveProvider(config: Config, apiKey: string | undefined, fetchImpl?: typeof fetch, ai?: WorkersAIBinding): TutorProvider {
  ...
  if (config.providerName === 'workers_ai' && ai) {
    const turns = createWorkersAITurnProvider({ ai, model: config.model, timeoutMs: config.providerTimeoutMs, gateway: { id: 'default', skipCache: true } });
