...
| | |
|---|---|
| Android package | `com.joinanny.littlebuddy` |
...
`adb uninstall com.joinanny.littlebuddy` (this deletes the save) before it will
...
/Users/hotkhwan/Library/Android/sdk/platform-tools/adb shell monkey \
  -p com.joinanny.littlebuddy -c android.intent.category.LAUNCHER 1
```
...
|---|---|---|
| `INSTALL_FAILED_UPDATE_INCOMPATIBLE` / `signatures do not match` | An older Little Days is installed, signed with a different key. Android never lets a debug key overwrite another key. | `adb uninstall com.joinanny.littlebuddy` then install again. **This deletes the save.** |
...
$ADB shell run-as com.joinanny.littlebuddy \
...
```sh
ADB=/Users/hotkhwan/Library/Android/sdk/platform-tools/adb
$ADB shell run-as com.joinanny.littlebuddy cat files/speech_diag.json
